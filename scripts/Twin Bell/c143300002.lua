--Twin Bell Grant Bearham
local s,id,o=GetID()
function s.initial_effect(c)
	--special summon Bearham
...
function s.spfilter(c,e,tp)
	return c:IsCode(143300001) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function s.dtg2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE+LOCATION_REMOVED) 
	and chkc:IsControler(tp) and s.dfilter(chkc) end
	if chk==0 then return Duel.IsPlayerCanDraw(tp,1)
		and Duel.IsExistingTarget(s.dfilter,tp,LOCATION_GRAVE+LOCATION_REMOVED,0,3,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
	local g=Duel.SelectTarget(tp,s.dfilter,tp,LOCATION_GRAVE+LOCATION_REMOVED,0,3,3,nil)
	Duel.SetOperationInfo(0,CATEGORY_TODECK,g,g:GetCount(),0,0)
	Duel.SetOperationInfo(0,CATEGORY_DRAW,nil,0,tp,1)
end
function s.dop2(e,tp,eg,ep,ev,re,r,rp)
